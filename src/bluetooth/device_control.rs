use crate::bluetooth::{
    LIGHTHOUSE_CHAR_UUID, LIGHTHOUSE_SERVICE_UUID, POWERON_COMMAND, STANDBY_COMMAND,
};
...
#[allow(dead_code)]
pub async fn power_on_lighthouses() -> Result<(), Box<dyn Error>> {
    // Default to non-JSON output for internal calls
    power_on_lighthouses_with_json(false).await
}

/// Power on lighthouses with JSON output control
pub async fn power_on_lighthouses_with_json(json_output: bool) -> Result<(), Box<dyn Error>> {
    log("Powering on lighthouses...", json_output);

    // Check if we have cached devices
    let cached_devices = crate::config::load_devices()?;

    if cached_devices.is_empty() {
        log(
            "No cached Lighthouse devices found. Performing a scan...",
            json_output,
        );
        return crate::bluetooth::scanning::scan_process_and_save_with_json(
            POWERON_COMMAND,
            json_output,
        )
        .await;
    ...
    // Initialize Bluetooth to connect to cached devices
    ...
    // Start a scan to find the cached devices
    adapter
    ...
    // Find the peripherals that match our cached devices
    let mut lighthouse_devices = Vec::new();

    for peripheral in peripherals.iter() {
        let address = peripheral.address().to_string();

        // Check if this peripheral is in our cached list
        if cached_devices
            .iter()
    ...
    if lighthouse_devices.is_empty() {
        ...
        log("Performing a new scan to find devices...", json_output);
        return crate::bluetooth::scanning::scan_process_and_save_with_json(
            POWERON_COMMAND,
            json_output,
        )
        ...
        log(
            &format!(
                "Found {} of {} cached devices in the current scan",
        ...
        // Send the power on command to the found devices
        return handle_device_command_with_json(&lighthouse_devices, POWERON_COMMAND, json_output)
            .await;
    }
}
...
#[allow(dead_code)]
pub async fn standby_lighthouses() -> Result<(), Box<dyn Error>> {
    // Default to non-JSON output for internal calls
    standby_lighthouses_with_json(false).await
}

/// Put lighthouses in standby mode with JSON output control
pub async fn standby_lighthouses_with_json(json_output: bool) -> Result<(), Box<dyn Error>> {
    ...
    if lighthouse_devices.is_empty() {
        ...
        // Send the standby command to the found devices
        return handle_device_command_with_json(&lighthouse_devices, STANDBY_COMMAND, json_output)
            .await;
    }
}
