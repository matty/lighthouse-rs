...
use bluetooth::{POWERON_COMMAND, STANDBY_COMMAND};
use cli::{
    ...
    STEAMVR_STOPPED_ARG, UNREGISTER_STEAMVR_ARG,
};
use config::{load_devices, load_devices_with_json};
    ...
    match load_devices_with_json(json_output) {
        Ok(devices) => {
            if !devices.is_empty() {
                ...
                match bluetooth::scan_process_and_save_with_json(0xFF, json_output).await {
                    Ok(_) => {
